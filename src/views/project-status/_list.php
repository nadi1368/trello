...
<div class="list list-created"  id="<?= 'list-'.$status->id; ?>" data-id="<?= $status->id ?>" data-ajax-url="<?= Url::to(['project-status/move', 'id'=>$status->id]) ?>">
    <header>
        <a class="list-title" data-type="text" data-pk="<?= $status->id; ?>" data-url="<?= Url::to(['project-status/update', 'id'=>$status->id]); ?>"   data-placement="right" data-title="<?= Module::t("module","Update Title") ?>"><?= Html::encode($status->title_status); ?></a>
    ...
    <ul class="connectedSortable" id="<?= 'tasks-ul-'.$status->id; ?>" data-status="<?= $status->id; ?>">
        <?= $this->render('/project/_list_task',['status' => $status]); ?>
    </ul>
    <footer>
