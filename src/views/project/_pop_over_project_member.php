...
<div class="pop-over-content"  id="project-member-list">
    <div class="list-group without-border  rtl">
                    ...
                    <tr>
                        <td width="20px">
                            <?php if($member->is_creator!==$is_creator && $project->isAdmin($user_id)): ?>
                                <a href="#" class="delete" data-ajax-url="<?= Url::to(['project/remove-member', 'id'=>$project->id, 'project_user'=>$member->id]) ?>" title="<?= Yii::t("app","Delete This Member") ?>"  onclick="return memberProject(this);">
                        ...
                        <td width="20px">
                            <?php if($member->is_creator==$is_creator )://اگر ایجاد کننده پروژه باشد کسی اجازه تغیر رل را ندارد ?>
                                <span class="is_admin"  title="<?= Yii::t("app","Role Is Admin") ?>">
                                    <i class="fa fa-check-square-o"></i>
                                </span>
                            <?php elseif($project->isAdmin($user_id)):// اگر کاربر سطح دسترسی ادمین داشت ?>
                                    <?php if($member->role==$role_admin): ?>
                                        <a href="#" class="is_admin" title="<?= Yii::t("app","Change Role To User") ?>" data-ajax-url="<?= Url::to(['project/change-role-member', 'id'=>$project->id, 'project_user'=>$member->id]) ?>"  onclick="return changeRoleProjectMember(this);">
                                            <i class="fa fa-check-square-o"></i>
                                        </a>
                                    <?php else: ?>
                                        <a href="#" class="is_user" title="<?= Yii::t("app","Change Role To Admin") ?>" data-ajax-url="<?= Url::to(['project/change-role-member', 'id'=>$project->id, 'project_user'=>$member->id]) ?>"  onclick="return changeRoleProjectMember(this);">
                            ...
                            <?php else: ?>
                                ...
                                <?php else: ?>
                                    <span class="is_user" title="<?= Yii::t("app","Role Is User") ?>">
                                        <i class="fa fa-square-o"></i>
                                    </span>
                                <?php endif; ?>
                            <?php endif; ?>
        ...
        <hr />
        <input type="tex" id="search-project-member-input" data-ajax-url="<?= Url::to(['project/search-member', 'id'=>$project->id]) ?>" class="form-control rtl" placeholder="<?= Yii::t("app","Search Members") ?>">
